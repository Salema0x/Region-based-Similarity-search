...
def tf_inference(imgs_b64, protocol, host, port, modelname):
    """
    Send batch of base64 images to tf-serving rest api and return results
    ...
    res = requests.post('%s://%s:%d/v1/models/%s:predict' % (protocol, host, port, modelname), data=data, headers={"content-type": "application/json"})
